...
export function useColumnResizeSave({ table, updateConfigChange }: UseColumnResizeSaveProps) {
  ...
  useEffect(() => {
    if (!isResizing && hasResizedRef.current && columnSizeTimeoutRef.current === null) {
      ...
      columnSizeTimeoutRef.current = setTimeout(async () => {
        ...
        if (Object.keys(allColumnSizes).length > 0 && sizesString !== lastSavedSizesRef.current) {
