...
import { LensDebugProvider, useLensDebugClient } from "@lens2/contexts/lens-debug-context";
...
function LensContent({ onError }: { onError?: (error: Error) => void }) {
  const { views, isLoading, error, setRecordsLoaded, enableViews } = useLensContext();
  ...
  return (
    <div className="flex flex-col h-full">
      <ViewProvider view={activeView}>
        {enableViews && <Toolbar activeViewId={activeViewId} onViewChange={handleViewChange} />}
        <div ref={viewContainerRef} className="flex flex-col flex-1 relative overflow-hidden">
          <FilterToolbar />
          <ViewContainer />
        </div>
...
export function Lens({ query, baseUrl, headers, onError, enableDebug = false, globalContext = {}, enableViews = true, viewScoping }: LensProps) {
