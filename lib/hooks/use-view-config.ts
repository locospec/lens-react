...
export function useViewConfig() {
  ...
  const openConfig = useCallback((panel?: string) => {
  ...
  const navigateToPanel = useCallback((panel: string) => {
    setActiveConfigPanel(panel);
  }, [setActiveConfigPanel]);
  ...
  const updateConfigChange = useCallback(async (key: string, value: any) => {
